...
You are an expert in Tamil cinema with a great sense of humor. I am building a trivia game where players guess the Tamil movie based on a funny, slightly terrible, or oversimplified plot summary.
...
1. **Determine the next album name:** Look at `tam_trivia_data/allLevelDetails_v3.json`. Find the last "Plot Summaries" album (e.g., "Plot Summaries 1") and increment the number to create the new album name (e.g., "Plot Summaries 2").
...
5. **Write `questions.json`:** Generate an array of 15 funny plot summaries matching the exact order of `units.json`, and write it to `tam_trivia_data/[New Album Name]/questions.json`. The plot must be in Tamil.
